...
def conv_relu_BN_forward(x,w,b,gamma,beta,conv_param,bn_param):
  a,conv_cache = conv_forward_fast(x,w,b,conv_param)
  s,relu_cache = relu_forward(a)
...
def conv_relu_BN_backward(dout,cache):
  ...
  ds, dgamma, dbeta= spatial_batchnorm_backward(dout,bn_cache)
  da = relu_backward(ds,relu_cache)
  dx,dw,db=conv_backward_fast(da,conv_cache)
...
def conv_relu_pool_BN_forward(x,w,b,gamma,beta,conv_param,pool_param,bn_param):
  ...
  a,conv_cache = conv_forward_fast(x,w,b,conv_param)
  s,relu_cache = relu_forward(a)
  d,pool_cache = max_pool_forward_fast(s,pool_param)
  out,bn_cache = spatial_batchnorm_forward(d, gamma, beta, bn_param)
...
def conv_relu_pool_BN_backward(dout,cache):
  ...
  dd,dgamma,dbeta=spatial_batchnorm_backward(dout,bn_cache)
...
def affine_relu_BN_backward(dout, cache):
  ...
  ds,dgamma,dbeta=batchnorm_backward(dout,bn_cache)
  da=relu_backward(ds,relu_cache)
  dx,dw,db=affine_backward(da,fc_cache)
